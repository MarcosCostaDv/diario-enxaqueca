...
def _metricas(dia: pd.DataFrame, cri: pd.DataFrame, hoje: pd.Timestamp) -> None:
    inicio = min([x for x in (dia["data_referencia"].min() if len(dia) else None,
                              cri["aura_inicio_local"].min().normalize() if len(cri) else None) if x is not None],
                 default=hoje)
    ...
    ultimo = (hoje - cri["aura_inicio_local"].max().normalize()).days if len(cri) else "—"
...
def _linha_com_crises(dia: pd.DataFrame, cri: pd.DataFrame, coluna: str, titulo: str, dominio: list[int]):
    ...
    camadas = [linha]
    if len(cri):
        marcas = cri[["aura_inicio_local"]].rename(columns={"aura_inicio_local": "aura"})
        camadas.append(alt.Chart(marcas).mark_rule(color=LARANJA, strokeWidth=2, strokeDash=[4, 3]).encode(
            x="aura:T", tooltip=[alt.Tooltip("aura:T", title="Início da aura", format="%d/%m %H:%M")]))
...
def render(id_usuario: str, fuso: str) -> None:
    ...
    if len(cri):
        ...
        por_mes = cri.assign(mes=cri["aura_inicio_local"].dt.to_period("M").dt.to_timestamp()) \
    ...
    if len(dia):
        st.markdown("**Registros diários** · linhas laranja tracejadas = início da aura")
    ...
    if len(cri):
        st.markdown("**Crises**")
        tab = pd.DataFrame({
            "Aura": cri["aura_inicio_local"].dt.strftime("%d/%m/%Y %H:%M"),
            "Precisão": cri["aura_precisao"],
            "Duração aura (min)": cri["aura_duracao_min"].map(lambda v: ">60" if v == 61 else v),
            "Aura→dor (min)": ((cri["dor_inicio_local"] - cri["aura_inicio_local"]).dt.total_seconds() / 60).round(),
            "Dor máx": cri["dor_max"],
            "Duração total (h)": ((cri["crise_fim_local"] - cri["aura_inicio_local"]).dt.total_seconds() / 3600).round(1),
